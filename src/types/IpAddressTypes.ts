export type IpV4 = string & { readonly __brand: unique symbol };

export const isIpV4 = (ip: string): ip is IpV4 => {
	const iPV4Regex = /^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/;
	return iPV4Regex.test(ip);
}

export type IpV6 = string & { readonly __brand: unique symbol };

export const isIpV6 = (ip: string): ip is IpV6 => {
	// yes it is strange to use the official definition since why would anyone ask if localhost in ipv6 is a part of the abuse db, but technical correctness is a feature, not a bug. This will be up for consideration for removal if people are not happy with this
	const iPV6Regex = /^(?:(?:[0-9a-fA-F]{1,4}:){7,7}[0-9a-fA-F]{1,4}|(?:[0-9a-fA-F]{1,4}:){1,7}:|(?:[0-9a-fA-F]{1,4}:){1,6}:[0-9a-fA-F]{1,4}|(?:[0-9a-fA-F]{1,4}:){1,5}(?::[0-9a-fA-F]{1,4}){1,2}|(?:[0-9a-fA-F]{1,4}:){1,4}(?::[0-9a-fA-F]{1,4}){1,3}|(?:[0-9a-fA-F]{1,4}:){1,3}(?::[0-9a-fA-F]{1,4}){1,4}|(?:[0-9a-fA-F]{1,4}:){1,2}(?::[0-9a-fA-F]{1,4}){1,5}|[0-9a-fA-F]{1,4}:(?:(?::[0-9a-fA-F]{1,4}){1,6})|:(?:(?::[0-9a-fA-F]{1,4}){1,7}|:)|fe80:(?::[0-9a-fA-F]{0,4}){0,4}%[0-9a-zA-Z]{1,}|::(?:ffff(?::0{1,4}){0,1}:){0,1}(?:(?:25[0-5]|(?:2[0-4]|1{0,1}[0-9]){0,1}[0-9])\.){3,3}(?:25[0-5]|(?:2[0-4]|1{0,1}[0-9]){0,1}[0-9])|(?:[0-9a-fA-F]{1,4}:){1,4}:(?:(?:25[0-5]|(?:2[0-4]|1{0,1}[0-9]){0,1}[0-9])\.){3,3}(?:25[0-5]|(?:2[0-4]|1{0,1}[0-9]){0,1}[0-9]))$/;
	return iPV6Regex.test(ip);
}

/** An IPv4 or IPv6 network in CIDR notation, such as `127.0.0.1/24`. */
export type Cidr = string & { readonly __brand: unique symbol };

export const isCidr = (network: string): network is Cidr => {
	const slash = network.lastIndexOf("/");
	if (slash === -1) {
		return false;
	}

	const address = network.slice(0, slash);
	const prefix = network.slice(slash + 1);

	// A prefix length is decimal, without a sign, without leading zeroes and
	// within the address family's width.
	if (!/^(?:0|[1-9][0-9]{0,2})$/u.test(prefix)) {
		return false;
	}

	const length = Number(prefix);
	if (isIpV4(address)) {
		return length <= 32;
	}
	return isIpV6(address) && length <= 128;
};
